...
img(Ventana, Imagen):-			       %cargar una imagen en la interfaz ventana 
...
a:- new(V, dialog('Cuestionario para un ingeniero electrico')),
    ...
    send(V, display, Texto, point(10, 60)),
    send(V, display, Texto2, point(10, 100)),
...
preguntas(Lista) :-
    length(Lista, TotalPreguntas),
    preguntas(Lista, TotalPreguntas).

preguntas([], Total) :-
    puntuacion(P),
    Porcentaje is round((P * 100) / Total),
    format(atom(Resultado), 'Tu puntuacion fue: ~w%', [Porcentaje]),
    mostrar_resultado(Resultado).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Modulo de preguntas tipo "A/B/C" %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
preguntas([[Pregunta, item(V1, L1), item(V2, L2), item(V3, L3)]|Resto], Total) :-
