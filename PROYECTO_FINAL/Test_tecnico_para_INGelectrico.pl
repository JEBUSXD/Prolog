...
preguntas([], Total) :-
    ...
    format(atom(Resultado), 'Tu puntuacion fue: ~w%', [Porcentaje]),%simbolo ~w se sustituye por Porcentaje
...
mostrar_resultado(Resultado) :-
    ...
    limpiar :- retractall(puntuacion(_)).%Reinicia el sistema de puntuación.
